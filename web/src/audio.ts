...
/**
 * 自動再生の制限を外すためだけの、ごく短い無音（8kHz / 16bit / 50ms）。
 *
 * **サンプルの入っていない WAV を使わないこと。** 44 バイトのヘッダだけの
 * ファイル（`data` の長さが 0）は、デコーダから見ると長さが決まらない。
 * デスクトップの Chrome は黙って「再生した」ことにするが、**スマートフォン
 * では読み込みに失敗して `play()` が拒否され**、「このブラウザでは音を
 * 鳴らせませんでした」だけが出る —— 本当は鳴らせるのに。
 */
const SILENCE =
