...
音声で会話し、Discord からチャット・リマインダー・日常タスクの依頼を受ける。会話を重ねるほど自分でスキルを増やしていくことを目指す。

> **ステータス: 実装フェーズ 4 まで完了・未リリース。**
> フェーズ 3.5（リマインダー・Web 検索）と 4（スキル自己改善・人格の記録）はデプロイ済みですが、**機能そのものはまだ実機で確認していません。**
...
- **どこからでも頼める**: Discord のテキストからリマインダー・タスクを依頼できる
...
- **その場で起票できる**: スレッドで「Issue にして」と頼むと、元投稿を出典にして GitHub の Issue が立つ
...
```
   ...
   Discord テキスト      │  agent/  Flue アプリ         │──▶ LLM プロキシ（LiteLLM / OpenAI 互換）
...
**残しておきたい通知には宛先を添えます**（→ D-31）。`channel` に**設定に置いた宛先の名前**（チャンネル ID ではありません）を書くと、通話中かどうかに関わらずそのチャンネルへ必ずテキストで残り、同じサーバの VC にいれば併せて読み上げます。**チャンネルへ残る文面は LLM の書き換えを通しません** —— 書き換えは読み上げを自然にするためのもので、記録の数値や固有名詞を変えてよい理由にはならないからです。設定に無い名前は 400 で弾きます（既定の配信先へは回しません）。宛先は `settings.yaml` の `notification.channels` に置きます。

**「ビデオ通話」の実現方式について**: Discord Bot は公式 API で映像を送信できません（音声のみ）。そのため映像は Discord へ流さず、**アバターは自前の Web ページで描画し、音声だけを Discord VC でやりとりする**構成を採ります。非公式手段（selfbot 等）は採用しません。詳細と根拠は [`docs/requirements/02-constraints.md`](docs/requirements/02-constraints.md) にあります。
