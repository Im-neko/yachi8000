...
**⚠ 文字起こしはローカルへ移す（→ D-48、2026-09-23 オーナー判断）。** `faster-whisper-small` をクラスタ内の CPU で回す（開発機で 1.5 秒、本番ノード換算で 2 秒前後）。**2 秒の下限は消える**（さくらの受け口の性質だった）ので、**Q-28 の引き伸ばしは作らない**。ノードに GPU は無い（→ C-06）。

**デプロイは 2 段階に分ける。** agent は起動時に `STT_MODEL` の実在を `/model/info` で検証して落ちるので、**サーバを立てる PR と `sttModel` を切り替える PR を分ける**（同じ PR だと ArgoCD が両方を同時に動かし、agent が一時的に起動できない）。
