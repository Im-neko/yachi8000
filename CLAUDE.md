...
| 6 | アバター（F-20〜F-23, F-62） | **スライス 1・2 デプロイ済み**（映す + 口が動く + ブラウザでも鳴る）。設定 UI（F-61）は未着手 |
| 7 | 音声入力 | 未着手 |
...
**⚠ フェーズ 6（アバター）は 2026-09-20 にデプロイ済み（`version: c8eac12`）。PVC 上の VRM（`/data/avatar.vrm`）と `settings.yaml` の `avatar` 節も置いた。** 実機で確認したのは「4 つのエンドポイントが応答すること」までで、**ブラウザで映ること・口が動くこと・音が鳴ることは誰も見ていない**（表情のブレンドは未検証の当て推量 → D-38）。
...
**⚠ テナント分離は 2026-09-20 に廃止した（→ D-35）。未デプロイ。** 人格・長期記憶・スキル・リマインダーは全体でひとつになり、代わりに**話者**（`SpeakerId`）が軸になった（→ F-05）。デプロイすると:

- **ランタイム状態 DB のテーブルが作り直される**（リマインダー・スキル・人格差分の中身は消える。起動ログに `Dropped a tenant-scoped table` の WARN が出る）
- **長期記憶（pgvector）は列を落とすだけで、中身は残る**
