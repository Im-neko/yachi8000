...
**⚠ 表情（F-24）は実装済みで未デプロイ。** 判断は Jev（型付きの判断だけを返すモデル）に頼む（→ D-41）。**クラスタ側の前提は未反映**: Sealed Secret の `JEV_API_KEY`（**任意なので、無くても起動する** —— 素の顔のまま動き、起動ログに WARN が 1 行出るだけ）。
...
**まだ誰も見ていないのは「ブラウザで実際に顔が変わること」。** 口形と食い合わないこと（→ D-41 の 2）も含めて、実機で見るまで未確認。
