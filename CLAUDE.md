...
**実装フェーズ・未リリース。フェーズ 4 まで実装済みで、自宅 Kubernetes クラスタで稼働中（2026-09-11〜）。フェーズ 3.5・4 は 2026-09-12 にデプロイ済みだが、機能そのものは実機未確認。フェーズ 6（アバター）はスライス 2 まで実装済みで、まだデプロイしていない。**
...
| 6 | アバター（F-20〜F-23, F-62） | **スライス 1・2 実装済み・未デプロイ**。映す + 口が動く + ブラウザでも鳴る。設定 UI（F-61）は未着手 |
| 7 | 音声入力 | 未着手 |
