...
| 6 | アバター（F-20〜F-25, F-61, F-62） | **デプロイ済み**（映す + 口が動く + ブラウザでも鳴る + 表情 + 身振り）。**設定 UI（F-61）は実装済み・未デプロイ。** 身振りは素材（VRMA）が未配置で、置くまで出ない |
| 7 | 音声入力 | 未着手 |
...
**ページと `/api/v1/avatar/*`・`/api/v1/settings*` の認証は ingress の forward auth で行う（→ D-37）。** 配線は GitOps リポジトリ側にあり、**アプリ側にはコードが 1 行も無い**。`/api/v1/notify`・`/api/v1/voice/status`・`/api/v1/health` だけは別 Ingress で素通しにしてある（Bearer で自分を守っているもの + 死活監視）。**ここに path を足すことは「インターネットから素通しで届く」という意味になる。**
