...
**ローカルの文字起こしは CPU で動くことを実測済み**（→ Q-29 の表）。`faster-whisper-small` が開発機で 1.5 秒、hvr1 換算で 2 秒前後。**2 秒の下限はローカルには無い**（さくらの受け口の性質だった）ので、移せば Q-28 の引き伸ばしは不要になる。**hvr1 に GPU は無い**（→ Q-02 決着）。

**常用に入れるには「従量課金（0.5 円 / 60 秒）」か「ローカルへ移す」かの判断が要る。オーナー待ち。**
