...
| 7 | 音声入力（F-13, F-14）+ カメラ（F-26） | **経路 B（ブラウザのマイク）の 1 段目とカメラを実装済み・未デプロイ。** 手動のボタンで 1 発話を送る。**常時聞き取り（VAD）と短い発話の引き伸ばしは未着手** |
...
**⚠ 音声入力（F-13 の経路 B）は実装済み・未デプロイ。クラスタ側の前提が 2 つとも未反映**で、どちらも入れるまでページの「話しかける」は失敗する:

- `STT_MODEL`（values.yaml。`sakura-whisper-large-v3-turbo`）。**任意の環境変数なので、無くても起動はする** —— 叩かれたときに 503 を返し、起動ログには WARN が 1 行出る
- PVC 上の `settings.yaml` の `web.speakers`（認証基盤の利用者名 → `discord-user-<id>`。→ D-45）。**ここに無い人は 403 で断られる**
