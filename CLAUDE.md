...
| 7 | 音声入力（F-13, F-14）+ カメラ（F-26） | **経路 B（ブラウザのマイク）の 1 段目とカメラをデプロイ済み**（`version: 1c9a1b4`）。手動のボタンで 1 発話を送る。**常時聞き取り（VAD）と短い発話の引き伸ばしは未着手** |
...
**⚠ 残る前提は 1 つ。PVC 上の `settings.yaml` の `web.speakers`**（認証基盤の利用者名 → `discord-user-<id>`。→ D-45）。**これを置くまで「話しかける」は 403 で断られる。** 覚えている話者は `discord-user-262923638641131520`（neko）。

**⚠ 文字起こしのエンジンが落ちている**（2026-09-22 夕方から。→ Q-29）。直前まで 579 ms で返していた同じ音声が、20 秒待っても返らない。**復旧するまでは、対応表を置いても「いま聞き取れません」になる。**
