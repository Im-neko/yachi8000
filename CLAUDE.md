...
**実装フェーズ・未リリース。フェーズ 4 まで実装済みで、自宅 Kubernetes クラスタで稼働中（2026-09-11〜）。フェーズ 3.5・4 は 2026-09-12 にデプロイ済みだが、機能そのものは実機未確認。フェーズ 6（アバター）は表情・身振りまでデプロイ済み（`version: 219d20c`）。設定 UI（F-61）だけが未デプロイ。**
