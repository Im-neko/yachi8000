...
**実装フェーズ・未リリース。フェーズ 4 まで実装済みで、自宅 Kubernetes クラスタで稼働中（2026-09-11〜）。フェーズ 3.5・4 は 2026-09-12 にデプロイ済みだが、機能そのものは実機未確認。フェーズ 6（アバター）はスライス 2 まで 2026-09-20 にデプロイ済み（`version: c8eac12`）。**
...
| 6 | アバター（F-20〜F-24, F-62） | **スライス 1・2 デプロイ済み**（映す + 口が動く + ブラウザでも鳴る）。**スライス 3（表情 F-24）は実装済み・未デプロイ。** 設定 UI（F-61）は未着手 |
| 7 | 音声入力 | 未着手 |
...
**⚠ フェーズ 6（アバター）は 2026-09-20 にデプロイ済み（`version: c8eac12`）。PVC 上の VRM（`/data/avatar.vrm`）と `settings.yaml` の `avatar` 節も置いた。** 実機で確認したのは「4 つのエンドポイントが応答すること」までで、**ブラウザで映ること・口が動くこと・音が鳴ることは誰も見ていない**（口形と表情の重ね方は実機の VRM で `overrideMouth` を確かめて決め直した → D-41 の 2）。

**⚠ 表情（F-24）は実装済みで未デプロイ。** 判断は Jev（型付きの判断だけを返すモデル）に頼む（→ D-41）。**クラスタ側の前提は未反映**: Sealed Secret の `JEV_API_KEY`（**任意なので、無くても起動する** —— 素の顔のまま動き、起動ログに WARN が 1 行出るだけ）。**確信度の閾値（`MIN_CONFIDENCE = 0.5`）は実測前の暫定値。** 鍵を通したら、淡々とした発話で返る確信度を測って D-41 に書き、決め直すこと。

**ページと `/api/v1/avatar/*` の認証は ingress の forward auth で行う（→ D-37）。** 配線は GitOps リポジトリ側にあり、**アプリ側にはコードが 1 行も無い**。`/api/v1/notify`・`/api/v1/voice/status`・`/api/v1/health` だけは別 Ingress で素通しにしてある（Bearer で自分を守っているもの + 死活監視）。**ここに path を足すことは「インターネットから素通しで届く」という意味になる。**
...
agent/           Flue アプリ本体（TypeScript）
  src/
    ...
    infrastructure/    port の実装
      ...
      expression/      Jev（型付きの判断モデル）で表情を判断する（→ D-41）
