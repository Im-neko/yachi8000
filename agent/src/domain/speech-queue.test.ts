...
describe('createSpeechQueue', () => {
  it('同じ優先度なら投入順に読む', () => {
    ...
    queue.enqueue({ priority: 'reply', sentences: ['A1', 'A2'] });
    queue.enqueue({ priority: 'reply', sentences: ['B1'] });
    expect(drain(queue)).toEqual(['A1', 'A2', 'B1']);
  });

  it('通知は残りの応答より先に読む（文の切れ目で割り込む）', () => {
    const queue = createSpeechQueue(10);
    queue.enqueue({ priority: 'reply', sentences: ['R1', 'R2', 'R3'] });
  ...
  it('割り込み中に確定した応答は、通知を読み終えてから読む', () => {
    const queue = createSpeechQueue(10);
    queue.enqueue({ priority: 'notification', sentences: ['N1', 'N2'] });
  ...
  it('上限を超える投入は丸ごと断る', () => {
    ...
    expect(queue.enqueue({ priority: 'reply', sentences: ['C', 'D'] })).toBe(
      false,
    );
  ...
  it('peek は take と同じオブジェクトを返す（先読みの同一性判定に使う）', () => {
    const queue = createSpeechQueue(10);
    queue.enqueue({ priority: 'reply', sentences: ['A'] });
