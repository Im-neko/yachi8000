...
| `GUILD_MESSAGE_REACTIONS` | `1 << 10` | **スキル承認のリアクション**（F-44）。サーバのチャンネル側 |
| `DIRECT_MESSAGE_REACTIONS` | `1 << 13` | 同上の **DM** 側。**片方だけだと「DM では押せるのにサーバでは反応しない」** |
...
### スキルの承認フロー（F-41, F-43, F-44）

- **Add Reactions** — **必須**。リアクションで承認・却下する形になった（→ F-44）。
  **Bot が自分で ✅ / ❌ を先に付ける**ので、これが無いと問いかけは出るが押す印が出ない
  （人が自分で同じ絵文字を付ければ動くが、何で答えるのか分からない）
- メッセージのボタン・セレクトメニューは **Send Messages だけで動く**（追加権限は不要）

> **partial の取り扱いに注意。** 承認は数分後・数日後に押される。そのころには
> メッセージが discord.js のキャッシュに無いので、`Partials.Reaction` を許さないと
> **イベントごと届かない**（コード側で指定済み）。
