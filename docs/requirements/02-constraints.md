...
### C-06. 本番ホストはヘッドレス Linux。GPU は無い

**確認済み**（2026-09-23）。ヘッドレス SSH 環境であることはグローバル CLAUDE.md に明記。**クラスタのノード（1 台）に GPU は無い** —— `kubectl get nodes` の capacity に `nvidia.com/gpu` が出ない。**CPU は Intel Xeon E5-2643 v3（24 スレッド）、メモリ 128GB。**

**開発機との差は 1 コアあたり約 1.27 倍**（同じ行列積を 1 スレッドで回して 0.28 秒 vs 0.22 秒）。**ローカルで重いモデルを回す見積もりは、この係数で換算する。**

**この制約の帰結**: 3D 描画をサーバサイドで行う設計（ヘッドレスレンダリング → 映像配信）は採らない。アバターはブラウザの WebGL に閉じる（→ D-02）。**ローカル STT は CPU 前提**でしか成り立たない（速さの実測は Q-29）。
